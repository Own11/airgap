...
def process_meeting(meeting_id: int) -> None:
    ...
    try:
        ...
        database.update_meeting(meeting_id, status="failed")


@router.post("/{meeting_id}/process", status_code=202)
async def start_processing(meeting_id: int, background_tasks: BackgroundTasks) -> dict[str, object]:
    database = SQLiteDatabase(get_settings())
    meeting = database.get_meeting(meeting_id)
    if meeting is None:
...
async def get_meeting(meeting_id: int) -> dict[str, object]:
