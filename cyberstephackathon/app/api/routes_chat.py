...
async def chat_about_meeting(
    ...
    prompt = f"Ответь на вопрос только по транскрипту встречи. Если ответа нет, скажи об этом.\nТРАНСКРИПТ:\n{context}\nВОПРОС: {payload.question}"
    try:
        async with httpx.AsyncClient(base_url=get_settings().ollama_base_url, timeout=120) as client:
            response = await client.post("/api/generate", json={"model": get_settings().ollama_model, "prompt": prompt, "stream": False})
